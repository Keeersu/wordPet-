...
```
用户选择答案 → 错误
  → 用户选项红色高亮（100ms），不显示正确答案
  → 底部弹窗从下滑入：显示鼓励文案（不暴露正确答案，保留再试机会）
  → 用户点击「再试一次 ↩」收起弹窗，选项恢复默认，可重新作答
```
...
|------|----------|------|
| 答对 | ✅ 鼓励文案 | 「下一题 →」|
| 答错第1次 | ❌ 错误提示 + 鼓励文案（不显示答案） | 「再试一次 ↩」|
| 答错第2次 | ❌ 错误提示 + 正确答案 + 词义/例句 + 鼓励文案 | 「下一题 →」|
...
| 背后内容 | 题目页半透明遮罩 `rgba(0,0,0,0.3)`，不可点击 |
| 关闭方式 | **仅手动点击按钮**，无自动关闭计时 |

**鼓励文案库（答对/答错随机轮换，避免重复）：**

```
答对：「完美！🎉」/ 「太棒了！」/ 「Momo 为你骄傲！」/ 「就是这样！」
答错：「没关系，继续加油！🐾」/ 「差一点～再来！」/ 「记住了，下次一定会！」
```
...
| 状态 | 选项背景色 | 文字色 |
|------|------------|--------|
| 默认 | `var(--color-bg)` | `var(--color-text)` |
| 答对 | `#66BB6A`（`--color-success`） | `#fff` |
| 答错（用户选项） | `#EF5350`（`--color-error`） | `#fff` |
| 正确答案高亮（第2次答错时） | `#66BB6A`（`--color-success`） | `#fff` |

- 每题最多 2 次机会；答错 2 次不扣分，计入错题记录；顶部进度条实时显示 `x/10`
