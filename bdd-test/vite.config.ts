...
export default defineConfig({
  ...
  test: {
    // Limit workers for 8 CPU / 8G CI: each fork ~0.6–0.7G, 5 workers peak ~3.3G. Use 4 or 2 if OOM.
    maxWorkers: 5,
    reporters: 'default',
    environment: 'jsdom',
    environmentOptions: {
    ...
    setupFiles: [
      ...
      path.join(__dirname, './src/polyfills/setupPolyfills.ts'),
      path.join(__dirname, './src/setupTeardown.ts')
    ],
    env: {
