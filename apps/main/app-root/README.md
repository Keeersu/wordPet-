...
└── bdd-test/
    ...
    │   ├── authSetup.ts                # Auth test helpers (seedTestUsers, signIn*, signOut)
...
import { apiFetch, FormError } from '@paraflow-ai/frontend-libs'
...
import { seedTestUsers, signInWithRegularUserA, signInWithRegularUserB } from '@bdd-test/authSetup'
import { pageLinks } from '@frontend/pageLinks'

beforeEach(async () => { await seedTestUsers() })
...
3. **`renderStore` (discouraged — last resort only)** — renders the current store state into jsdom without re-executing loaders, runs assertions in a callback, then automatically unmounts. Most bugs should be caught at the Store level; only use `renderStore` when the bug is in the component's rendering logic itself (e.g., conditional display, wrong text). Optionally pass `{ saveHtml: '/tmp/debug.html' }` as the third argument to save for inspection.
...
Multi-word route sub-paths use camelCase (e.g., `/createPost`, not `/create-post`) for Hono RPC dot notation access. kebab-case routes require bracket notation, and `typeof api['kebab-key'].$get` inside `InferResponseType<>` generics triggers TypeScript parser error TS1005.
...
| `helper.renderStore(url, callback, options?)` | Render current store state to jsdom, run assertions in callback, then unmount |
| `helper.TEST_BASE_TIME` | Fixed test time (2025-01-15 12:00 UTC) |
| `helper.TEST_DEADLINES.IN_7_DAYS` | Future deadline for testing |

**Auth helper functions** (available only after enabling auth integration — call `seedTestUsers()` in `beforeEach`):

| Function | Purpose |
|----------|---------|
| `seedTestUsers()` | Seed admin + register test users into DB (call in `beforeEach`) |
| `signInWithRegularUserA()` | Sign in as regular user A (auto-resets Store) |
| `signInWithRegularUserB()` | Sign in as regular user B (auto-resets Store) |
| `signInWithAdmin()` | Sign in as admin (auto-resets Store) |
| `signOut()` | Sign out and reset Store |

**Note:** `signInWithXxx()` functions automatically call `helper.store.reset()` internally, so you rarely need to call `helper.store.reset()` directly.
