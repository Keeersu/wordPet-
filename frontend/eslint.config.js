...
export default tseslint.config([
  globalIgnores(['dist']),
  // Base rules for ALL TypeScript files (no type-aware)
  {
    files: ['**/*.{ts,tsx}'],
    extends: [
  ...
  // Type-aware rules for src/ and scripts/ only (requires TypeScript project)
  {
    files: ['src/**/*.{ts,tsx}', 'scripts/**/*.{ts,tsx}'],
    languageOptions: {
      parserOptions: {
        projectService: {
          allowDefaultProject: ['scripts/*.ts', 'scripts/*.tsx', 'scripts/utils/*.ts'],
        },
